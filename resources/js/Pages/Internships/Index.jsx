...
export default function Index() {
  const { internships = [], filters = {} } = usePage().props;
  ...
  const handleSearch = (searchTerm) => {
    ...
    const filtered = internships.filter((internship) => {
      ...
      const startDate = internship.start_date?.toLowerCase() || '';
      const endDate = internship.end_date?.toLowerCase() || '';
                      ...
                      {filteredInternships.map((internship) => (
                        <tr key={internship.id}>
                          ...
                          <td className="whitespace-nowrap px-6 py-4">
                            ...
                            <button
                              onClick={() =>
                                confirmInternshipDeletion(internship)
                              }
      ...
      <Modal show={!!deletingInternship} onClose={closeModal}>
        <div className="p-6">
          ...
          <p className="mt-1 text-sm text-gray-600">
            Once this internship is deleted, all of its resources and data will
            be permanently deleted.
          </p>
          <div className="mt-6 flex justify-end">
            <SecondaryButton onClick={closeModal}>Cancel</SecondaryButton>
            <DangerButton className="ml-3" onClick={deleteInternship}>
              Delete Internship
            </DangerButton>
          </div>
        </div>
