...
export default function Index() {
  const { courses = [], filters = {} } = usePage().props;
  ...
  const handleSearch = (searchTerm) => {
    ...
    const filtered = courses.filter((course) => {
      ...
      const completionDate = course.completion_date?.toLowerCase() || '';

      return (
        name.includes(searchLower) ||
        institution.includes(searchLower) ||
        completionDate.includes(searchLower)
      );
    });
      ...
      <Modal show={!!deletingCourse} onClose={closeModal}>
        <div className="p-6">
          ...
          <p className="mt-1 text-sm text-gray-600">
            Once this course is deleted, all of its resources and data will be
            permanently deleted.
          </p>
          <div className="mt-6 flex justify-end">
            <SecondaryButton onClick={closeModal}>Cancel</SecondaryButton>
            <DangerButton className="ml-3" onClick={deleteCourse}>
              Delete Course
            </DangerButton>
          </div>
        </div>
